...
2. We introduced `CodexClient::manage_and_connect_ws` and `Codex::manage_and_connect_ws`, which perform the daemon auto-start and management logic before connecting.
...
- Users relying on `connect_ws` to auto-start local servers must migrate to `manage_and_connect_ws`.
